...
Both _infrastructure server_ and _master nodes_ is also given a _floating IP_
address on the public network. This provides direct access to the
_infrastructure server_ from where you can access all nodes by SSH.
_Master nodes_ have _floating IP_ assigned to make sure these nodes are
accessible when an _external loadbalancer_ is used for accessing Openshift
services.
