...
* Neutron LBaaS - this loadbalancer is used by default. Neutron loadbalancer
  ...
  loadbalancer has to be used for it. This is default option, but can be set
...
* External loadbalancer - a user is expected to set its own loadbalancer both
  for master nodes and Openshift routers. This is suggested type for production.
...
* Dedicated loadbalancer node - a dedicated node is created during stack
  creation and HAProxy loadbalancer is configured on it. This type is useful for
  demo/testing purposes only because HA is not assured for the single loadbalancer
  To select this type include `-e openshift-on-openstack/env_loadbalancer_dedicated.yaml`
  when creating the stack.
  node.
